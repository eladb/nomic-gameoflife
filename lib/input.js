...
// read input lines from `rl` in challange format. lightly validates input.
// `rl` is a `readline` interface.
// `callback` is a `function(err, config)` where `config` has: `width`, `height`, `matrix` and `iterations`.
