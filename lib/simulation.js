//
// creates a game of life simulation object based on `config`
// `config.matrix` - the life matrix
// `config.width` - width of matrix
// `config.height` - height of matrix
//
// object returned has two methods:
// `sim.iterate()` performs a single iteration.
// `sim.write_matrix(output)` writes the current matrix to an output stream.
//
